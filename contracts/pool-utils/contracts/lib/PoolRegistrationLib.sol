...
library PoolRegistrationLib {
  function registerPool(
    ...
    return
      registerPoolWithAssetManagers(vault, specialization, tokens, new address[](tokens.length));
  }
  ...
  function deregisterToken(IVault vault, bytes32 poolId, IERC20 token) internal {
    IERC20[] memory tokens = new IERC20[](1);
    tokens[0] = token;
